      ...
      <div className={cn(hideNav && "hidden")}>
        <TabsList aria-label={localization.settings.settings}>
          <TabsTrigger
            value="account"
            nativeButton={false}
            render={<Link href={`${basePaths.settings}/${viewPaths.settings.account}`} className="gap-1" />}
          >
          ...
          <TabsTrigger
            value="security"
            nativeButton={false}
            render={<Link href={`${basePaths.settings}/${viewPaths.settings.security}`} className="gap-1" />}
          >
            ...
            (plugin) =>
              plugin.settingsTabs?.map((settingsTab, index) => (
                <TabsTrigger
                  ...
                  nativeButton={false}
                  render={<Link href={`${basePaths.settings}/${plugin.viewPaths?.settings?.[settingsTab.view]}`} className="gap-1" />}
                >
                  {settingsTab.label}
                </TabsTrigger>
              )) ?? []
