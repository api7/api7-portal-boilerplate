...
test.describe('Test Gateway Product with DCR', () => {
  ...
  test('test gateway product with dcr crud', async ({
    ...
    await test.step('create product with dcr', async () => {
      await a7UICreateGatewayProduct(a7UIPage, productName, gatewayName, serviceName, 'public', false, a7Ctx, 'dcr', dcrProviderName, portalHost);
    });
    ...
    await test.step('edit oauth client', async () => {
      ...
      await page.locator('#redirect_uris_0_redirect_url').fill('localhost:3000');
      await page.locator('#desc').fill('test desc 2');
      await page.locator('.ant-drawer-footer').locator('button', { hasText: 'Save' }).click();

      // check update data save correctly
      await page.getByRole('button', { name: 'Edit' }).click();
      await expect(page.locator('#redirect_uris_0_redirect_url')).toHaveValue('localhost:3000');
  ...
  test('test gateway product with wrong dcr provider issuer', async ({
    ...
    await test.step('create product with wrong dcr provider', async () => {
      await a7UICreateGatewayProduct(a7UIPage, productName, gatewayName, serviceName, 'public', false, a7Ctx, 'dcr', invalidDcrProviderName, portalHost);
    });
  ...
  test('test search oauth client', async ({
    page,
    a7Ctx,
    a7UIPage
  }) => {
    ...
    await test.step('go to application page to create oauth client', async () => {
      ...
      const clientId1 = await page.locator('.ant-space-item', { hasText: 'Client ID' })
        .locator('input')
        .inputValue();
      ...
      const clientId2 = await page.locator('.ant-space-item', { hasText: 'Client ID' })
        .locator('input')
        .inputValue();

      // test search oauth client
      await page.locator('[placeholder="Search Description"]').fill('test desc');
      await page.locator('[placeholder="Search Description"]').press('Enter');
      await expect(page.locator('.ant-table-cell', { hasText: clientId1 })).toBeVisible();
      await expect(page.locator('.ant-table-cell', { hasText: clientId2 })).toBeVisible();

      await page.locator('[placeholder="Search Description"]').fill('test desc 1');
      await page.locator('[placeholder="Search Description"]').press('Enter');
      await expect(page.locator('.ant-table-cell', { hasText: clientId1 })).toBeVisible();
      await expect(page.locator('.ant-table-cell', { hasText: clientId2 })).toBeHidden();

      await page.locator('[placeholder="Search Description"]').fill('test desc 2');
      await page.locator('[placeholder="Search Description"]').press('Enter');
      await expect(page.locator('.ant-table-cell', { hasText: clientId2 })).toBeVisible();
      await expect(page.locator('.ant-table-cell', { hasText: clientId1 })).toBeHidden();

      await page.locator('[placeholder="Search Description"]').fill('non-existent');
      await page.locator('[placeholder="Search Description"]').press('Enter');
      await expect(page.locator('.ant-table-cell', { hasText: clientId1 })).toBeHidden();
      await expect(page.locator('.ant-table-cell', { hasText: clientId2 })).toBeHidden();
    });
