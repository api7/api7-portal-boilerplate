...
import { API_PROVIDER_LOGIN_OPTIONS, API_DCR_PROVIDERS } from './constant';
...
export const a7PostLoginOption = async (
  ...
  const postLoginOption = await ctx.post(`${API_PROVIDER_LOGIN_OPTIONS}?portal_id=${portalID}`, { data });
...
export const a7GetLoginOptions = async (ctx: A7Ctx) => {
  const portalID = await a7DefaultPortalID(ctx)
  const getLoginOptions = await ctx.get(`${API_PROVIDER_LOGIN_OPTIONS}?portal_id=${portalID}`);
...
export const a7DeleteLoginOptionByName = async (
  ...
  const portalID = await a7DefaultPortalID(ctx)
  const getAllLoginOptions = await ctx.get(`${API_PROVIDER_LOGIN_OPTIONS}?portal_id=${portalID}`);
...
export const a7DeleteDCRProviderList = async (ctx: A7Ctx) => {
  ...
  return await Promise.allSettled(allIds.map((id) => ctx.delete(`${API_DCR_PROVIDERS}/${id}`)));
};
